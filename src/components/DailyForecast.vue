...
const computedDay = (unix: number) => {
   const day = new Date((unix + storeState.data.timezone_offset) * 1000).getDay()
   return weekdays[day]
}
</script>

<template>
   <section class="daily component">
      <div class="day" v-for="(day, index) in storeState.data.daily.slice(0, 7)" :key="index">
         <div class="wrapper">
